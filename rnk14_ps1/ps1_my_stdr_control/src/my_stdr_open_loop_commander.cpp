...
int main(int argc, char **argv) {
    ...
    double half_speed = 0.5; // 0.5m/s speed command
    //double yaw_rate = 0.5; //0.5 rad/sec yaw rate command
    double half_yaw_rate = 0.25; //0.25 rad/sec yaw rate command
    double time_3_sec = 3.0; // should move 3 meters or 1.5 rad in 3 seconds
    double time_6_sec = 6.0; // 6 second timer
    ...
    twist_cmd.linear.x=half_speed; //command to move forward
    while(timer<time_6_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.angular.z=half_yaw_rate; //and start spinning in place
    ...
    twist_cmd.linear.x=half_speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<time_6_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    twist_cmd.linear.x=0.0; //stop moving forward
    twist_cmd.angular.z= -1.0 * half_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_6_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    twist_cmd.linear.x=half_speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<(time_6_sec + time_3_sec)) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=half_speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<(time_6_sec + time_3_sec + 0.5)) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
