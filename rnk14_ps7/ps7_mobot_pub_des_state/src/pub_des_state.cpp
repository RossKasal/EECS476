...
void DesStatePublisher::initializePublishers() {
    ...
    // Add something to publish the velocity
    vel_cmd_publisher_ = nh_.advertise<geometry_msgs::Twist>("/cmd_vel" , 1); //find this in the header
}
...
void DesStatePublisher::pub_next_state() {
    ...
    if (e_stop_trigger_) {
        ...
        trajBuilder_.build_braking_traj(current_pose_, des_state_vec_); //this is the other function that we just built
    ...
    vel_cmd_publisher_.publish(current_des_state_.twist.twist);
}
