...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    // Get the current state in front of the robot
    nav_msgs::Odometry current_state;
    current_state = vec_of_states.front();
    // Clear the states from the robot because the trajectory is changing
    vec_of_states.clear();

    // Create our variables
    double x_current = current_state.pose.pose.position.x;
    double y_current = current_state.pose.pose.position.y;
    double vel_current = sqrt(current_state.twist.twist.linear.x * current_state.twist.twist.linear.x); //this is an absolute value so really it is a speed
    double psi_current = current_state.pose.pose.orientation.z;

    // Making the new trajectory
    nav_msgs::Odometry des_state = current_state;
    des_state.twist.twist.angular.z = 0; //stop spinning this is a simple method that does not work perfectly should slow down then stop

    // Slow down
    while(vel_current > 0) {
    	vel_current -= accel_max_*dt_; //just like the ramp up but instead slowing down in steps
    	des_state.twist.twist.linear.x = vel_current;
    	x_current += vel_current * dt_ * cos(psi_current); //slow down the x
    	y_current += vel_current * dt_ * sin(psi_current); //slow down the y
    	des_state.pose.pose.position.x = x_current; //make this slow down go into the pose
    	des_state.pose.pose.position.y = y_current; //make this slow down go into the pose
    	vec_of_states.push_back(des_state); //push the pose back
    }

    des_state.twist.twist = halt_twist_; //ensure that the halt is actually 0
}

//main fnc of this library: constructs a spin-in-place reorientation toacceleration constraints
...
void TrajBuilder::build_point_and_go_traj(geometry_msgs::PoseStamped start_pose,
    ...
    geometry_msgs::PoseStamped bridge_pose; //lidar_alarm end of prev traj to start of new traj
